...
public class ClienteService {
    ...
    public ClienteResponse crea(ClienteRequest clienteRequest) {
        Cliente cliente = new Cliente(clienteRequest.getNome(), clienteRequest.getEmail());
        Cliente salvato = clienteRepository.save(cliente);

        return new ClienteResponse(salvato.getId(),
                salvato.getNome(), salvato.getEmail());
    }
    ...
    public ClienteResponse trovaPerId(Long id) {
        Cliente cliente = clienteRepository.findById(id).orElseThrow(() -> new ClienteNonTrovatoException(id));
        return new ClienteResponse(cliente.getId(),
                cliente.getNome(), cliente.getEmail());
    }

    @Transactional
    public ClienteResponse modificaCliente(Long id, ClienteRequest clienteRequest) {
        Cliente cliente = clienteRepository.findById(id).orElseThrow(() -> new ClienteNonTrovatoException(id));
    ...
    public void elimina(Long id) {
        ...
        Cliente cliente = clienteRepository.findById(id).orElseThrow(() -> new ClienteNonTrovatoException(id));
        log.info("Cliente id={} trovato, procedo con l'eliminazione", id);
        clienteRepository.delete(cliente);
        log.info("Cliente eliminato correttamente, id={}", id);
    }
    ...
    public List<ClienteResponse> trovaPerStringa(String nome) {
        ...
        return clienteRepository.findByNomeContainingIgnoreCase(nome).stream()
                .map(cliente -> new ClienteResponse(cliente.getId(), cliente.getNome(), cliente.getEmail())).toList();
    }

    public List<ClienteResponse> trovaPerEmail(String email) {
        return clienteRepository.cercaPerEmail(email).stream()
                .map(cliente -> new ClienteResponse(cliente.getId(), cliente.getNome(), cliente.getEmail())).toList();
    ...
    public ClienteDettaglioResponse trovaClienteConOrdini(Long id) {
        ...
        List<OrdineResponse> ordini = cliente.getOrdini().stream().map(ordine -> new OrdineResponse(
                ordine.getId(), ordine.getStato(), ordine.getCliente().getId())).toList();
